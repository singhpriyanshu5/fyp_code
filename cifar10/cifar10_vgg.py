...
lr = tf.train.exponential_decay(initial_learning_rate, global_step, 25*int(cifar10_input.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN/BATCH_SIZE), 0.96, staircase=True)
...
with tf.Session() as sess:
  ...
  train_writer = tf.summary.FileWriter('./tensorboard_logs/train_vgg', sess.graph)
...
#Rescaling the validation data to the range 0 to 1
  ...
  degrees_vector = np.random.random_integers(-90, 90, size=(cifar10_input.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL,))
  test_d_rotated = []
  for i in range(cifar10_input.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL):
       x_image = test_d[i]
  ...
  degrees_vector = np.random.random_integers(-90, 90, size=(cifar10_input.NUM_EXAMPLES_PER_EPOCH_FOR_VAL,))
  val_d_rotated = []
  for i in range(cifar10_input.NUM_EXAMPLES_PER_EPOCH_FOR_VAL):
      x_image = val_d[i]
      x_image_padded = np.lib.pad(x_image, ((7,7),(7,7),(0,0)), 'symmetric')
