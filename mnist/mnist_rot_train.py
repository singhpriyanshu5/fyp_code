...
import mnist_rot_main

parser = parser = argparse.ArgumentParser()

parser.add_argument('--train_dir', type=str, default='/tmp/mnist_train',
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    # inference model.
    logits = mnist_rot_main.inference(images)

    # Calculate loss.
    loss = mnist_rot_main.loss(logits, labels)

    # Build a Graph that trains the model with one batch of examples and
    # updates the model parameters.
    train_op = mnist_rot_main.train(loss, global_step)
...
def main(argv=None):  # pylint: disable=unused-argument
  ## can be removed for mnist
  # cifar10.maybe_download_and_extract()

  ##Remove this section in order to continue training from checkpoint
  if not FLAGS.continue_checkpoint:
