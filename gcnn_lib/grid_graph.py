...
def grid_graph(grid_side,number_edges,metric):
    ...
    z = grid(grid_side)#gives a grid of the given size
...
def adjacency(dist, idx):
    ...
    It gives us an adjacency matrix of size MxM with the matrix entries such
    that for each row all column indices are zero except the closest distance indices which have the
    distance values as the matrix entries. """
    M, k = dist.shape
