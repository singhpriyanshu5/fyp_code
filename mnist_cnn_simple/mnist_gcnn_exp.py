...
train_rotated = tf.reshape(train_rotated, [-1, train_data.shape[1]])
...
#test_rotated = tf.reshape(test_rotated, [-1, test_data.shape[1]])
...
train_data = train_rotated
#test_data = test_rotated
...
print("train_data max",np.amax(train_data))
print("train_data min",np.amin(train_data))
print("test_data max",np.amax(test_data))
print("test_data min",np.amin(test_data))

# Reindex nodes to satisfy a binary tree structure
train_d = perm_data(train_data, perm)
...
#  x_image = np.squeeze(train_images[i])
...
#      scipy.misc.toimage(x_image).show()
  #x_image_rotated = scipy.misc.imrotate(x_image, 0, 'cubic')
 # x_cropped = x_image_rotated[6:34,6:34]
#  train_d_rotated.append(x_image)
#train_d = np.stack(train_d_rotated)
#train_d = tf.reshape(train_d, [-1, 784])
#train_d = sess.run(train_d)
...
for epoch in range(num_epochs):  # loop over the dataset multiple times
    ...
    while len(indices) >= batch_size:
        ...
        x_s, x_t,  _,acc_train,loss_train = sess.run([x_score, x_target, backward,evaluation,loss], feed_dict={x: batch_x, x_target: batch_y, d: dropout_value})
        if epoch == 0:
            #print("x_score", x_s[:10])
            #print("x_target", x_t[:10])
            print("x_score isnan", np.isnan(x_s).any())
            #print("target isnan", np.isnan(x_t).any())
